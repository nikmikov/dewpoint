...
declinationOfSunAngle n = let n' = fromIntegral n
                          in 0.98565 * (n' + 10.0) + 1.914 * sin (toRad (0.98565 * (n' - 2)) )

-- | calculate solar hour angle in radians - angle from solar noon
solarHourAngle :: Lon -> UTCTime -> Double
solarHourAngle (Lon lon) t = toRad $ lon - (360.0 * secondsSinceMidnight t / secondsInADay  + 180.0)
...
temperatureDiffPerSecond coord t cloudCover = let rawSolarInflux = solarEnergyInflux coord t
                                                  albedo = 0.3 + (0.6 * cloudCover) -- heuristic formula
                                                  absorbedSolarInflux = (1.0 - albedo) * rawSolarInflux
                                              in absorbedSolarInflux / 1000.0 -- experimental formula
